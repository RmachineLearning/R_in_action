...
###########################################
#2.3 创建一个有继承关系的RC类
# 创建RC类User #fields: 定义属性和属性类型
User<-setRefClass("User",fields=list(name="character"))

# 创建User的子类Member, #contains: 定义父类，继承关系
...
##############################################
#2.4 RC对象的默认值
#RC的类有一个指定构造器方法 $initialize()，这个构造器方法在实例化对象时，
#会自动被运行一次，通过这个构造方法可以设置属性的默认值。
# 定义一个RC类
...
#在S3,S4的对象系统中，我们实现对象的行为时，都是借助于泛型函数来实现的。
#这种现实方法的最大问题是，在定义时函数和对象的代码是分离的，需要在运行时，通过判断对象的类型完成方法调用。而在RC对象系统中，方法可以定义在类的内部，通过实例化的对象完成方法调用。
# 定义一个RC类，包括方法
User<-setRefClass("User",
